...
let compare_ cmp m1 m2 =
  let rec compare__aux e1 e2 =
      match (e1, e2) with
      (End, End) -> 0
    | (End, _)  -> -1
    | (_, End) -> 1
    | (More(v1, d1, r1, e1), More(v2, d2, r2, e2)) ->
        ...
        let c = cmp d1 d2 in
        if c <> 0 then c else
        compare__aux (cons_enum r1 e1) (cons_enum r2 e2)
  in compare__aux (cons_enum m1 End) (cons_enum m2 End)
